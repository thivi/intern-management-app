			...
			.then((response) => {
				...
				profiles.forEach((profile, index: number) => {
					...
					const internProfile: Profile = {
						...
						Degree: profile[3],
						Joined_date: new Date(profile[4]).toLocaleDateString(),
						Leaving_date: new Date(profile[5]).toLocaleDateString(),
			...
			{showInternProfile ? (
				...
				<>
					<Paper variant="elevation" className={classes.listPaper}>
						...
						<Box className={classes.broadListWrapper}>
							<List className={`${classes.list} ${classes.broadList}`}>
								<ListItem className={classes.listHeader}>
									<Grid container spacing={2}>
								...
								{isLoading
									? listSkeletons()
									: !(interns?.length === 0 || filteredInterns?.length === 0) &&
