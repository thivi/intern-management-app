...
export const ProjectTasks = (): ReactElement => {
    ...
    const [sortOrder, setSortOrder] = useState<Sort>({ Title: true, PullRequest: true, Month: true });
    const [searchQuery, setSearchQuery] = useState("");
    const [deleteIndex, setDeleteIndex] = useState(-1);
    const [sorted, setSorted] = useState<Sort>({ Title: false, PullRequest: false, Month: false });
                ...
                response?.values?.forEach((issue: string[], index: number) => {
                    if (issue[0] === authState.authData.email) {
                        ...
                        issues.push({
                            ...
                            PullRequest: issue[2],
                            Month: issue[3]
                                ? new Date(issue[3]).toLocaleDateString("en-US", {
                                      month: "long",
                                      year: "numeric",
                                  })
                                : "",
    ...
    const editForm = useFormik({
        ...
        initialValues: {
            ...
            month: paginatedProjectTasks[editIndex]?.Month || new Date().toDateString(),
            ...
            <Paper className={classes.addPaper}>
                <form noValidate onSubmit={addForm.handleSubmit}>
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={8} md={6}>
                            <MuiPickersUtilsProvider utils={MomentUtils}>
                                <KeyboardDatePicker
                                    ...
                                    value={addForm.values.month}
                                    label="Month"
                                    fullWidth
                                    helperText={addForm.touched.month && addForm.errors.month && addForm.errors.month}
                                    onBlur={addForm.handleBlur}
                                    margin="normal"
                                    views={["month", "year"]}
                                    format="MM-YYYY"
                                    onChange={(date: Moment) => {
                                        addForm.setFieldValue("month", date);
                                    }}
                                    KeyboardButtonProps={{
                                        "aria-label": "change date",
                                    }}
                                    error={!!(addForm.touched.month && addForm.errors.month)}
                                />
                            </MuiPickersUtilsProvider>
                        </Grid>
                        <Grid item xs={4} md={6} className={classes.rightCenterFlex}>
                            <FormControlLabel
                                control={
                                    <Checkbox
                        ...
                        <Grid item xs={12} md={12} className={classes.addButtonGrid}>
                            <Button
                                            ...
                                            {editIndex === index ? (
                                                <Grid container item xs={10} md={9}>
                                                    <form onSubmit={editForm.handleSubmit} className={classes.gridForm}>
                                                        <Grid xs={4} item className={classes.gridRightMargin}>
                                                            <TextField
                                                                variant="standard"
                                                                name="title"
                                                                label="Title"
                                                        ...
                                                        <Grid xs={4} item>
                                                            <MuiPickersUtilsProvider utils={MomentUtils}>
                                                                <KeyboardDatePicker
                                                                    className={classes.editDatePicker}
                                                                    name="month"
                                                                    placeholder={"eg: " + new Date().toDateString()}
                                                                    value={editForm.values.month}
                                                                    label="Month"
                                                                    fullWidth
                                                                    helperText={
                                                                        editForm.touched.month &&
                                                                    ...
                                                                    format="MM-YYYY"
                                                                    onChange={(date: Moment) => {
                                                                        editForm.setFieldValue("month", date);
                                                                    }}
                                                                    KeyboardButtonProps={{
                                                                        "aria-label": "change date",
                                                                    }}
                                                ...
                                                <>
                                                    ...
                                                    <Grid container alignItems="center" item xs={4} md={3}>
                                                        <Typography component="h4">{gitIssue.Title}</Typography>
                                                    </Grid>
                                                    <Grid container alignItems="center" item xs={3} md={3}>
